...
const EmployeeScheduleCalendar = () => {
  ...
  const handleSchedule = (scheduleData) => {
    ...
    setEmployees((prevEmployees) => {
      const newEmployees = [...prevEmployees];
      const employeeIndex = newEmployees.findIndex(emp => emp.name === scheduleData.employeeName);
      if (employeeIndex !== -1) {
        newEmployees[employeeIndex].events.push({
          id: `event-${Date.now()}`,
          title: scheduleData.employeeName,
          job: "Scheduled Shift",
          start: new Date(`${scheduleData.date.toDateString()} ${scheduleData.startTime}`),
          end: new Date(`${scheduleData.date.toDateString()} ${scheduleData.endTime}`),
          color: "#60a5fa",
        });
      }
      return newEmployees;
